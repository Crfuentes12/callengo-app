...
The Finances tab provides a full P&L (Profit and Loss) view of the platform's financial position.
...
Fetches all data needed to render the six tabs. The endpoint runs multiple database queries and external API calls. As of the March 2026 audit, the hourly and daily call statistics queries are parallelized using `Promise.all()` to reduce response time.
...
| File | Description |
|------|-------------|
| `src/components/admin/AdminCommandCenter.tsx` | Main component (~1,200 lines, 6 tabs) |
