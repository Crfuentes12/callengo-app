                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={testCallStats.chartByDay} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        formatter={((v: unknown, name: string) => [v ?? 0, name === 'testCalls' ? 'Test Calls' : 'Regular Calls']) as any}
                      />
                  ...
                  {testCallStats.byStatus.length > 0 ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        ...
                        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                        <Tooltip formatter={((v: unknown, name: string) => [v ?? 0, name]) as any} contentStyle={{ fontSize: 12 }} />
                      </PieChart>
                    </ResponsiveContainer>
                  ) : (
                  ...
                  {testCallStats.avgDurationByAgent.length > 0 ? (
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={testCallStats.avgDurationByAgent} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 0 }}>
                        ...
                        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                        <Tooltip formatter={((v: unknown) => { const n = Number(v ?? 0); return [`${Math.floor(n / 60)}m ${n % 60}s`, 'Avg Duration']; }) as any} contentStyle={{ fontSize: 12 }} />
