...
function ChevronIcon({ className = "w-4 h-4", direction = "left" }: { className?: string; direction?: "left" | "right" }) {
  return (
...
export default function Sidebar({ company, onLogout }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Logo & Brand */}
      <div className={`
        flex items-center h-16 border-b border-slate-800/60
      ...
      `}>
        {!isCollapsed && (
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-lg tracking-tight">Callengo</span>
          </div>
        )}
        ...
        <div className="px-4 py-4 border-b border-slate-800/60">
          <div className="flex items-center gap-3">
            {company.favicon_url ? (
              ...
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-slate-300 font-semibold text-sm border border-slate-700/50">
        ...
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isCollapsed && (
                <div className="absolute left-full ml-3 px-2.5 py-1.5 bg-slate-800 text-white text-sm rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-xl border border-slate-700/50">
                  {item.name}
                </div>
              )}
            </Link>
      ...
      <div className="p-3 border-t border-slate-800/60">
        ...
        >
          <LogoutIcon className="w-5 h-5 shrink-0 group-hover:text-red-400 transition-colors" />
          {!isCollapsed && <span className="font-medium text-[14px]">Sign out</span>}
          {isCollapsed && (
            <div className="absolute left-full ml-3 px-2.5 py-1.5 bg-slate-800 text-white text-sm rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 shadow-xl border border-slate-700/50">
              Sign out
            </div>
