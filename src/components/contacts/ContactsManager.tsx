...
export default function ContactsManager({ initialContacts, initialTotalCount, initialContactLists = [], companyId, hasSalesforceAccess, sfConnected, hasHubSpotAccess = false, hsConnected = false, hasPipedriveAccess = false, pdConnected = false, gsConnected = false, hasClioAccess = false, clioConnected = false, hasZohoAccess = false, zohoConnected = false, hasDynamicsAccess = false, dynamicsConnected = false, hasSimplyBookAccess = false, sbConnected = false }: ContactsManagerProps) {
  ...
  const [contactsTipMinimized, setContactsTipMinimized] = useState(false);
  ...
  return (
    <div className="space-y-5">
      ...
      {contactsTourLoaded && contactsTipMinimized && (
        <div className="flex justify-end">
          <button
            onClick={() => setContactsTipMinimized(false)}
            title="Show tips"
      ...
      {contactsTourLoaded && !contactsTourSeen && !contactsTipMinimized && (
        <div className="relative rounded-xl overflow-hidden border border-[var(--color-primary)]/20 shadow-sm">
          ...
          <div className="relative px-5 py-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-3">
                  <p className="text-sm font-bold text-[var(--color-ink)]">Getting started with Contacts</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {[
                    { icon: 'M12 4.5v15m7.5-7.5h-15', label: 'Add manually', desc: 'Click "+ Add Contacts" → "Add Manually" to enter contacts one by one' },
                    { icon: 'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5', label: 'Import CSV / Excel', desc: 'Upload a spreadsheet — we auto-map your columns to contact fields' },
                    { icon: 'M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0H3', label: 'Google Sheets sync', desc: 'Connect a Google Sheet and keep contacts in sync automatically' },
                    { icon: 'M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z M6 6h.008v.008H6V6z', label: 'Organize with Lists', desc: 'Create lists to segment contacts for targeted campaigns' },
                    { icon: 'M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125', label: 'Custom fields', desc: 'Open any contact to add extra fields your agent can reference during calls' },
                    { icon: 'M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244', label: 'CRM sync', desc: 'Connect HubSpot, Salesforce or Pipedrive to pull contacts automatically' },
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-[var(--color-primary)]/10">
              <span className="text-[11px] text-[var(--color-neutral-400)]">These tips will always be here if you need them</span>
              <button onClick={dismissContactsTour} className="text-[11px] font-semibold text-[var(--color-primary)] hover:text-[var(--color-primary-700)] transition-colors hover:underline">
