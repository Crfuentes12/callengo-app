    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-[var(--border-default)] p-12 text-center">
          <div className="w-16 h-16 rounded-2xl bg-[var(--color-neutral-100)] text-black flex items-center justify-center mx-auto mb-4">
            <PipedriveIcon className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold text-[var(--color-ink)] mb-2">Connect Pipedrive</h2>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[var(--color-neutral-100)] text-black flex items-center justify-center">
            <PipedriveIcon className="w-5 h-5" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[var(--color-ink)]">Pipedrive Contacts</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[var(--color-neutral-50)] border-b border-[var(--border-default)]">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-[var(--color-neutral-600)]">Organization</th>
                  <th className="text-left px-4 py-3 font-semibold text-[var(--color-neutral-600)]">Deals</th>
                  <th className="text-left px-4 py-3 font-semibold text-[var(--color-neutral-600)]">Synced</th>
                  <th className="text-left px-4 py-3 font-semibold text-[var(--color-neutral-600)]">Updated</th>
                </tr>
              </thead>
                  ...
                  filteredPersons.map((person) => (
                    <React.Fragment key={person.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-medium text-[var(--color-ink)]">{person.name}</td>
                        <td className="px-4 py-3 text-[var(--color-neutral-600)]">{getPrimaryEmail(person) || '\u2014'}</td>
                        <td className="px-4 py-3 text-[var(--color-neutral-600)]">{getPrimaryPhone(person) || '\u2014'}</td>
                        <td className="px-4 py-3 text-[var(--color-neutral-600)]">{person.org_id?.name || '\u2014'}</td>
                        <td className="px-4 py-3 text-[var(--color-neutral-600)]">
                          <span className="text-xs">
                            ...
                            {person.closed_deals_count > 0 && (
                              <span className="text-[var(--color-neutral-50)]0">{person.closed_deals_count} closed</span>
                            )}
                            {person.open_deals_count === 0 && person.closed_deals_count === 0 && '\u2014'}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-[var(--color-neutral-50)]0 text-xs">{formatDate(person.update_time)}</td>
                      </tr>
                          ...
                          <td colSpan={9} className="bg-emerald-50 px-4 py-0">
                            <div className="pl-14 py-4">
                              <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">First Name</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.first_name || '\u2014'}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Last Name</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.last_name || '\u2014'}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Organization</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.org_id?.name || '\u2014'}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Owner</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.owner_id?.name || '\u2014'}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Open Deals</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.open_deals_count}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Closed Deals</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.closed_deals_count}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Status</span>
                                  <p className="text-[var(--color-neutral-700)]">{person.active_flag ? 'Active' : 'Inactive'}</p>
                                </div>
                                <div>
                                  <span className="text-[var(--color-neutral-400)] text-xs font-medium">Created</span>
                                  <p className="text-[var(--color-neutral-700)]">{formatDate(person.add_time)}</p>
                                </div>
                              </div>
                            </div>
                          </td>
