...
export default function SettingsManager({ company: initialCompany, settings: initialSettings, user }: SettingsManagerProps) {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'company' | 'calling' | 'billing'>(
    initialTab === 'calling' || initialTab === 'billing' || initialTab === 'company' ? initialTab : 'general'
  );
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      phSettingsEvents.companyProfileUpdated();
  ...
  return (
    <div className="space-y-6">
      {/* Success Message */}
      {success && (
        <div className="bg-[var(--color-success-50)]/80 border border-[var(--color-success-200)] rounded-xl p-4 flex items-center gap-3 animate-slideDown">
          <div className="w-8 h-8 rounded-lg bg-[var(--color-success-100)] flex items-center justify-center">
            <svg className="w-4 h-4 text-[var(--color-success-600)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <p className="text-sm text-[var(--color-success-700)] font-medium">{success}</p>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-[var(--border-default)]/80 overflow-hidden shadow-sm">
        <div className="border-b border-[var(--border-subtle)]">
          <nav className="flex">
            {[
              { id: 'general', label: t.settings.tabs.general, iconPath: 'M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
              { id: 'company', label: t.settings.tabs.company, iconPath: 'M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21' },
              { id: 'calling', label: t.settings.tabs.calling, iconPath: 'M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z' },
              { id: 'billing', label: t.settings.tabs.billing, iconPath: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z' },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => { setActiveTab(tab.id as 'general' | 'company' | 'calling' | 'billing'); settingsEvents.tabChanged(tab.id); phSettingsEvents.tabChanged(tab.id); }}
                className={`flex-1 px-6 py-4 text-sm font-medium border-b-2 transition-all flex items-center justify-center gap-2 ${
                  activeTab === tab.id
                    ? 'border-[var(--color-primary)] text-[var(--color-primary)] bg-[var(--color-primary-50)]'
                    : 'border-transparent text-[var(--color-neutral-600)] hover:text-[var(--color-ink)] hover:bg-[var(--color-neutral-50)]'
                }`}
              >
            ...
            <div className="space-y-6">
              ...
              <div className="relative overflow-hidden gradient-bg-subtle rounded-2xl p-8 shadow-md">
                <div className="relative z-10 flex items-center gap-6">
                  ...
                  <div className="relative group">
                    ...
                    <label
                      htmlFor="logo-upload"
                      className="absolute inset-0 flex items-center justify-center bg-[var(--color-neutral-900)]/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all cursor-pointer rounded-2xl"
                    >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[var(--color-neutral-600)] font-semibold">{t.settings.company.connected}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <form onSubmit={handleUpdateCompany} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-semibold text-[var(--color-neutral-700)] mb-2 flex items-center gap-2">
                      <span className="w-6 h-6 rounded-md bg-[var(--color-primary-50)] text-[var(--color-primary)] flex items-center justify-center"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg></span>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none transition-all bg-white hover:border-[var(--border-strong)]"
                      required
                    />
                  </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-[var(--color-neutral-700)] mb-2 flex items-center gap-2">
                    <span className="w-6 h-6 rounded-md bg-[var(--color-info-100)] text-[var(--color-info-600)] flex items-center justify-center"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" /></svg></span>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none transition-all bg-white hover:border-[var(--border-strong)]"
                    />
                    <button
                      type="button"
                  ...
                  <p className="text-xs text-[var(--color-neutral-500)] mt-2 flex items-center gap-1.5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-[var(--color-neutral-700)] mb-2 flex items-center gap-2">
                    <span className="w-6 h-6 rounded-md bg-[var(--color-success-100)] text-[var(--color-success-600)] flex items-center justify-center"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg></span>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none transition-all resize-none bg-white hover:border-[var(--border-strong)]"
                  />
                </div>
          ...
          {activeTab === 'general' && (
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-[var(--color-primary-50)] flex items-center justify-center">
                    <svg className="w-4 h-4 text-[var(--color-primary-600)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                  </div>
                  <h3 className="text-base font-bold text-[var(--color-ink)]">Profile</h3>
                </div>

                <div className="bg-[var(--color-neutral-50)] rounded-xl p-5 border border-[var(--border-default)] space-y-4">
                  {profileSuccess && (
                    <div className="bg-[var(--color-success-50)] border border-[var(--color-success-200)] rounded-lg px-3 py-2 text-sm text-[var(--color-success-700)] font-medium">{profileSuccess}</div>
                  )}
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1.5">Full Name</label>
                      <input
                        type="text"
                        value={fullName}
                        onChange={e => setFullName(e.target.value)}
                        className="w-full px-3 py-2.5 border border-[var(--border-default)] rounded-lg text-sm focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none bg-white"
                        placeholder="Your full name"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1.5">Email</label>
                      <div className="relative">
                        <input
                          type="email"
                          value={user.email}
                          disabled
                          className="w-full px-3 py-2.5 border border-[var(--border-default)] rounded-lg text-sm bg-[var(--color-neutral-100)] text-[var(--color-neutral-500)] cursor-not-allowed"
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-[var(--color-neutral-400)] bg-[var(--color-neutral-200)] px-1.5 py-0.5 rounded">Locked</span>
                      </div>
                    </div>
                  </div>
                  <button
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-[var(--color-error-50)] flex items-center justify-center">
                    <svg className="w-4 h-4 text-[var(--color-error-600)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></svg>
                  </div>
                  <h3 className="text-base font-bold text-[var(--color-ink)]">Security</h3>
                </div>

                {/* Change Password */}
                <div className="bg-[var(--color-neutral-50)] rounded-xl p-5 border border-[var(--border-default)] space-y-4">
                  <h4 className="text-sm font-semibold text-[var(--color-ink)]">Change Password</h4>
                  {passwordError && (
                    <div className="bg-[var(--color-error-50)] border border-[var(--color-error-200)] rounded-lg px-3 py-2 text-sm text-[var(--color-error-700)]">{passwordError}</div>
                  )}
                  {passwordSuccess && (
                    <div className="bg-[var(--color-success-50)] border border-[var(--color-success-200)] rounded-lg px-3 py-2 text-sm text-[var(--color-success-700)] font-medium">{passwordSuccess}</div>
                  )}
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1.5">New Password</label>
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-3 py-2.5 pr-10 border border-[var(--border-default)] rounded-lg text-sm focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none bg-white"
                        />
                        <button type="button" onClick={() => setShowNewPw(!showNewPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-neutral-400)] hover:text-[var(--color-neutral-600)]">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1.5">Confirm Password</label>
                      <input
                        type={showNewPw ? 'text' : 'password'}
                ...
                <div className="bg-[var(--color-neutral-50)] rounded-xl p-5 border border-[var(--border-default)] space-y-4">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-[var(--color-ink)] flex items-center gap-2">
                        ...
                        {!mfaLoading && mfaFactors.length > 0 && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--color-success-100)] text-[var(--color-success-700)] border border-[var(--color-success-200)]">
                            <svg className="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                            Enabled
                          </span>
                        )}
                      </h4>
                      <p className="text-xs text-[var(--color-neutral-500)] mt-0.5">Use an authenticator app for extra account protection.</p>
                    </div>
                    {mfaLoading ? (
                      <div className="w-5 h-5 border border-[var(--border-strong)] border-t-[var(--color-neutral-600)] rounded-full animate-spin" />
                    ) : mfaFactors.length > 0 ? (
                      <button
                        onClick={() => handleDisable2FA(mfaFactors[0].id as string)}
                        disabled={disabling2FA}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold text-[var(--color-error-600)] bg-[var(--color-error-50)] hover:bg-[var(--color-error-100)] border border-[var(--color-error-200)] transition-colors disabled:opacity-50"
                      >
                  ...
                  {mfaError && (
                    <div className="bg-[var(--color-error-50)] border border-[var(--color-error-200)] rounded-lg px-3 py-2 text-sm text-[var(--color-error-700)]">{mfaError}</div>
                  )}
                  {mfaSuccess && (
                    <div className="bg-[var(--color-success-50)] border border-[var(--color-success-200)] rounded-lg px-3 py-2 text-sm text-[var(--color-success-700)] font-medium">{mfaSuccess}</div>
                  )}
                    ...
                    <div className="space-y-4 pt-2 border-t border-[var(--border-default)]">
                      ...
                      <div className="flex flex-col sm:flex-row gap-5 items-start">
                        ...
                        <div className="space-y-3 flex-1">
                          <div>
                            <p className="text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1">Manual entry key</p>
                            <code className="block text-xs bg-[var(--color-neutral-100)] border border-[var(--border-default)] rounded-lg px-3 py-2 font-mono text-[var(--color-neutral-700)] break-all select-all">
                              {mfaEnrollData.secret}
                            </code>
                          </div>
                          <div>
                            <label className="block text-xs font-semibold text-[var(--color-neutral-600)] uppercase mb-1.5">Enter 6-digit code from app</label>
                            <div className="flex gap-2">
                              <input
                                type="text"
                                inputMode="numeric"
                                ...
                                placeholder="000000"
                                className="flex-1 px-3 py-2.5 border border-[var(--border-default)] rounded-lg text-sm font-mono tracking-widest focus:ring-2 focus:ring-[var(--color-primary-200)] focus:border-[var(--color-primary)] outline-none bg-white"
                              />
                              <button
                          ...
                          <button
                            ...
                            className="text-xs text-[var(--color-neutral-400)] hover:text-[var(--color-neutral-600)] transition-colors"
                          >
                            Cancel setup
                          </button>
                        </div>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        phNavigationEvents.languageChanged('previous', language);
              ...
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-7 h-7 rounded-lg bg-[var(--color-warning-50)] flex items-center justify-center">
                    <svg className="w-4 h-4 text-[var(--color-warning-600)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" /></svg>
                  </div>
                  <h3 className="text-base font-bold text-[var(--color-ink)]">{t.settings.notifications.title}</h3>
                </div>
