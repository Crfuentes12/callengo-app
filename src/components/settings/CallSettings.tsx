...
export default function CallSettings({ settings, onSettingsChange, onSubmit, loading, success, companyId, initialSection }: CallSettingsProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Plan Overview */}
      {plan && (
        <div className="gradient-bg-subtle rounded-xl p-6 border border-[var(--border-default)]">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-[var(--color-neutral-600)] mt-1">Your current subscription limits and usage</p>
            </div>
            {usageStats && (
              <div className="text-right">
                <div className="text-2xl font-bold text-[var(--color-primary)]">{usageStats.minutesUsed}/{usageStats.minutesIncluded}</div>
                <div className="text-xs text-[var(--color-neutral-600)] font-medium">{t.common.minutes} used</div>
              </div>
            )}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/80 backdrop-blur rounded-lg p-3 border border-[var(--border-default)]"><div className="text-xs text-[var(--color-neutral-600)] font-medium mb-1">{t.settings.calling.maxDuration}</div><div className="text-lg font-bold text-[var(--color-ink)]">{plan.max_call_duration} min</div></div>
            <div className="bg-white/80 backdrop-blur rounded-lg p-3 border border-[var(--border-default)]"><div className="text-xs text-[var(--color-neutral-600)] font-medium mb-1">Concurrent Calls</div><div className="text-lg font-bold text-[var(--color-ink)]">{plan.max_concurrent_calls}</div></div>
            {plan.max_calls_per_day && (<div className="bg-white/80 backdrop-blur rounded-lg p-3 border border-[var(--border-default)]"><div className="text-xs text-[var(--color-neutral-600)] font-medium mb-1">{t.settings.calling.maxCallsPerDay}</div><div className="text-lg font-bold text-[var(--color-ink)]">{plan.max_calls_per_day}</div></div>)}
            {usageStats && usageStats.averageCallDuration && (<div className="bg-white/80 backdrop-blur rounded-lg p-3 border border-emerald-100"><div className="text-xs text-[var(--color-neutral-600)] font-medium mb-1">Avg Call Duration</div><div className="text-lg font-bold text-emerald-600">{usageStats.averageCallDuration} min</div></div>)}
          </div>
          {usageStats && usageStats.totalCalls > 0 && (
            <div className="mt-4 bg-white/60 backdrop-blur rounded-lg p-3 border border-[var(--border-default)]">
              <div className="text-xs text-[var(--color-neutral-600)] font-medium mb-2">Monthly Usage</div>
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-[var(--color-neutral-200)] rounded-full h-2.5"><div className="gradient-bg h-2.5 rounded-full transition-all duration-500" style={{ width: `${Math.min((usageStats.minutesUsed / usageStats.minutesIncluded) * 100, 100)}%` }}></div></div>
                <div className="text-sm font-bold text-[var(--color-neutral-700)]">{Math.round((usageStats.minutesUsed / usageStats.minutesIncluded) * 100)}%</div>
              </div>
              <div className="text-xs text-[var(--color-neutral-500)] mt-2">{usageStats.totalCalls} calls this month</div>
            </div>
          )}
        </div>
      )}
        ...
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-5">
            ...
            <div className="flex items-start gap-3 flex-1">
              ...
              <div>
                <div className="flex items-center gap-2 mb-0.5">
                  <h4 className="text-sm font-bold text-[var(--color-ink)]">Auto-Rotating Numbers</h4>
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">Active</span>
                </div>
                <p className="text-xs text-[var(--color-neutral-500)]">Callengo rotates numbers from our pool to maximize pickup rates and avoid spam flags. Included on all plans.</p>
              </div>
            </div>
            {!isFreePlan && (
              <a
                ...
                className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[var(--color-neutral-900)] text-white text-xs font-semibold hover:bg-[var(--color-neutral-800)] transition-colors whitespace-nowrap"
              >
          ...
          {!isFreePlan && (
            <div className="mt-3 pt-3 border-t border-[var(--border-subtle)] flex items-center gap-2">
              ...
              <p className="text-xs text-[var(--color-neutral-500)]">Want your own number for better brand recognition? Add a <strong>Dedicated Number</strong> for $15/mo and calls always show your number.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border border-[var(--border-default)] rounded-xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3 flex-1">
            <div className="w-9 h-9 rounded-lg bg-[var(--color-neutral-100)] flex items-center justify-center flex-shrink-0 mt-0.5">
              <svg className="w-4 h-4 text-[var(--color-neutral-500)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
            </div>
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <h4 className="text-sm font-bold text-[var(--color-ink)]">Call Recordings</h4>
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--color-neutral-100)] text-[var(--color-neutral-500)]">30-day retention</span>
              </div>
              <p className="text-xs text-[var(--color-neutral-500)]">All calls are recorded automatically. Recordings are stored for 30 days then deleted.</p>
            </div>
          </div>
          <div className="flex-shrink-0">
            <div className="w-11 h-6 bg-[var(--color-neutral-300)] rounded-full relative opacity-60 cursor-not-allowed">
        ...
        <div className="mt-3 pt-3 border-t border-[var(--border-subtle)]">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-[var(--color-neutral-500)]">Upgrade to <strong className="text-[var(--color-neutral-700)]">Recording Vault ($12/mo)</strong> to keep recordings for 12 months, searchable & downloadable anytime.</p>
            </div>
            {!isFreePlan && (
              <a
                href="/settings?tab=billing"
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-6">
          ...
          <div>
            <div className="flex items-end justify-between mb-3">
              <label className="block text-sm font-bold text-[var(--color-neutral-700)]">{t.settings.calling.maxDuration}</label>
              <div className="text-right">
          ...
          <div className="bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 rounded-lg p-4">
            <div className="flex gap-3">
              ...
              <div className="text-sm text-[var(--color-ink)]"><p className="font-semibold mb-1">Duration Optimizer</p><p className="text-[var(--color-neutral-700)]">Lower duration = more calls per month.{plan && ` With ${plan.minutes_included} minutes included, you can make ~${approximateCalls} calls at ${settings.default_max_duration} min each.`}</p></div>
            </div>
          </div>
        </div>
        ...
        {/* Voice & Language */}
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-6">
          <h4 className="text-sm font-semibold text-[var(--color-ink)] flex items-center gap-2"><svg className="w-4 h-4 text-[var(--color-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>Voice & Language</h4>
          <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-3">{t.settings.calling.voice}</label><VoiceSelector selectedVoiceId={settings.default_voice} onVoiceSelect={(voiceId) => onSettingsChange({ ...settings, default_voice: voiceId })} /></div>
        </div>

        {/* Call Timing */}
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-6">
          <h4 className="text-sm font-semibold text-[var(--color-ink)] flex items-center gap-2"><svg className="w-4 h-4 text-[var(--color-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Call Scheduling</h4>
          <div className="grid md:grid-cols-2 gap-6">
            <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.interval} ({t.common.minutes})</label><input type="number" min="1" max="60" value={settings.default_interval_minutes} onChange={(e) => onSettingsChange({ ...settings, default_interval_minutes: parseInt(e.target.value) })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" /></div>
            <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.maxCallsPerDay}{plan && plan.max_calls_per_day && <span className="ml-2 text-xs text-[var(--color-primary)]">(Plan limit: {plan.max_calls_per_day})</span>}</label><input type="number" min="1" max={plan?.max_calls_per_day || 1000} value={settings.max_calls_per_day} onChange={(e) => onSettingsChange({ ...settings, max_calls_per_day: parseInt(e.target.value) })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" /></div>
          </div>
        </div>
        ...
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">Start Time</label><input type="time" value={settings.working_hours_start} onChange={(e) => onSettingsChange({ ...settings, working_hours_start: e.target.value })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" /></div>
            <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">End Time</label><input type="time" value={settings.working_hours_end} onChange={(e) => onSettingsChange({ ...settings, working_hours_end: e.target.value })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" /></div>
            <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.timezone}</label><select value={settings.timezone} onChange={(e) => onSettingsChange({ ...settings, timezone: e.target.value })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] bg-white outline-none transition-all cursor-pointer hover:border-[var(--border-strong)]"><option value="America/New_York">Eastern Time (ET)</option><option value="America/Chicago">Central Time (CT)</option><option value="America/Denver">Mountain Time (MT)</option><option value="America/Los_Angeles">Pacific Time (PT)</option><option value="America/Phoenix">Arizona (MST)</option><option value="America/Anchorage">Alaska (AKT)</option><option value="Pacific/Honolulu">Hawaii (HST)</option><option value="Europe/London">London (GMT)</option><option value="Europe/Paris">Paris (CET)</option><option value="Asia/Tokyo">Tokyo (JST)</option><option value="Australia/Sydney">Sydney (AEDT)</option></select></div>
          </div>
          <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-3">{t.settings.calling.workingDays}</label><div className="grid grid-cols-7 gap-2">{['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => { const dayLower = day.toLowerCase(); const isSelected = settings.working_days?.includes(dayLower) ?? true; return (<button key={day} type="button" onClick={() => { const currentDays = settings.working_days || ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']; const newDays = isSelected ? currentDays.filter(d => d !== dayLower) : [...currentDays, dayLower]; onSettingsChange({ ...settings, working_days: newDays }); }} className={`px-3 py-2 rounded-lg text-xs font-bold transition-all border ${isSelected ? 'gradient-bg text-white border-[var(--color-primary)] hover:opacity-90' : 'bg-white text-[var(--color-neutral-600)] border-[var(--border-default)] hover:border-[var(--border-strong)]'}`}>{day.slice(0, 3)}</button>); })}</div><p className="text-xs text-[var(--color-neutral-500)] mt-2">Select which days of the week calls should be placed</p></div>
          <div className="flex items-start gap-3 bg-[var(--color-neutral-50)] border border-[var(--border-default)] rounded-lg p-4"><input type="checkbox" id="exclude-holidays" checked={settings.exclude_holidays ?? false} onChange={(e) => onSettingsChange({ ...settings, exclude_holidays: e.target.checked })} className="mt-1 w-4 h-4 rounded border-[var(--border-strong)] text-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 cursor-pointer" /><div className="flex-1"><label htmlFor="exclude-holidays" className="text-sm font-bold text-[var(--color-neutral-700)] cursor-pointer block mb-1">{t.settings.calling.excludeHolidays}</label><p className="text-xs text-[var(--color-neutral-600)]">Automatically skip calls on major US federal holidays.</p></div></div>
        </div>
        ...
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-6">
          ...
          <p className="text-xs text-[var(--color-neutral-500)] -mt-4">Configure automatic voicemail detection and follow-up call behavior. Available from Starter plan and above.</p>

          {isFreePlan ? (
            <div className="bg-[var(--color-neutral-50)] border border-[var(--border-default)] rounded-lg p-6 text-center">
              <div className="w-12 h-12 rounded-full bg-[var(--color-neutral-100)] flex items-center justify-center mx-auto mb-3">
                <svg className="w-6 h-6 text-[var(--color-neutral-400)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              </div>
              <p className="text-sm font-semibold text-[var(--color-neutral-700)]">Upgrade to unlock Voicemail & Follow-ups</p>
              <p className="text-xs text-[var(--color-neutral-500)] mt-1">Available on Starter plan and above</p>
            </div>
          ) : (
            <div className="space-y-4">
              {/* Voicemail Toggle */}
              <div className="flex items-start gap-4 bg-[var(--color-neutral-50)] border border-[var(--border-default)] rounded-lg p-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <label htmlFor="voicemail-enabled" className="text-sm font-bold text-[var(--color-neutral-700)] cursor-pointer">{t.settings.calling.voicemail}</label>
                  ...
                  <p className="text-xs text-[var(--color-neutral-500)]">Automatically detect voicemail and leave a message. {plan && plan.slug === 'starter' ? 'Standard voicemail on Starter plan.' : 'Business+ plans can customize the voicemail message.'}</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer mt-1">
                  ...
                  <div className="w-11 h-6 bg-[var(--color-neutral-200)] peer-focus:ring-2 peer-focus:ring-[var(--color-primary)]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-[var(--border-strong)] after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[var(--color-primary)]"></div>
                </label>
              </div>
              ...
              <div className="flex items-start gap-4 bg-[var(--color-neutral-50)] border border-[var(--border-default)] rounded-lg p-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <label htmlFor="followup-enabled" className="text-sm font-bold text-[var(--color-neutral-700)] cursor-pointer">{t.settings.calling.followUp}</label>
                    {plan && plan.slug === 'starter' && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[var(--color-neutral-200)] text-[var(--color-neutral-600)]">Max 1</span>
                    )}
                  ...
                  <p className="text-xs text-[var(--color-neutral-500)]">Automatically schedule follow-up calls for contacts that didn&apos;t answer or requested a callback.</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer mt-1">
              ...
              {settings.followup_enabled && (
                <div className="ml-4 pl-4 border-l-2 border-[var(--color-primary)]/20 space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.followUp} Max Attempts</label>
                      <input type="number" min="1" max={plan && plan.slug === 'starter' ? 1 : 10} value={settings.followup_max_attempts ?? 1} onChange={(e) => onSettingsChange({ ...settings, followup_max_attempts: parseInt(e.target.value) })} className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" />
                      {plan && plan.slug === 'starter' && <p className="text-xs text-[var(--color-neutral-400)] mt-1">Starter plan limited to 1 follow-up</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.followUp} {t.settings.calling.interval} ({t.common.hours})</label>
                      ...
                      <p className="text-xs text-[var(--color-neutral-400)] mt-1">Time between follow-up attempts</p>
                    </div>
                  </div>
                  ...
                  {plan && plan.slug !== 'starter' && plan.slug !== 'free' && (
                    <div className="flex items-start gap-4 bg-purple-50 border border-purple-200 rounded-lg p-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <label htmlFor="smart-followup" className="text-sm font-bold text-[var(--color-neutral-700)] cursor-pointer">{t.settings.calling.smartFollowUp}</label>
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-100 text-purple-700">Business+</span>
                        </div>
                        <p className="text-xs text-[var(--color-neutral-500)]">When a contact requests a callback at a specific time (e.g., &quot;call me Friday at 5 PM&quot;), Callengo will automatically schedule the follow-up for that exact time instead of using the default interval.</p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer mt-1">
                        ...
                        <div className="w-11 h-6 bg-[var(--color-neutral-200)] peer-focus:ring-2 peer-focus:ring-purple-400/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-[var(--border-strong)] after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600"></div>
                      </label>
                    </div>
                  )}
                </div>
        ...
        {/* Test Phone */}
        <div className="bg-white border border-[var(--border-default)] rounded-xl p-6 space-y-4">
          <h4 className="text-sm font-semibold text-[var(--color-ink)] flex items-center gap-2"><svg className="w-4 h-4 text-[var(--color-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>Test Configuration</h4>
          <div><label className="block text-sm font-bold text-[var(--color-neutral-700)] mb-2">{t.settings.calling.testPhone}</label><input type="tel" value={settings.test_phone_number} onChange={(e) => onSettingsChange({ ...settings, test_phone_number: e.target.value })} placeholder="+1 (555) 123-4567" className="w-full px-4 py-3 border border-[var(--border-default)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all hover:border-[var(--border-strong)]" /><p className="text-xs text-[var(--color-neutral-500)] mt-2">Use this number for testing agent configurations before launching campaigns</p></div>
        </div>
