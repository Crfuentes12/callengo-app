...
export default function SocialAuthButtons({ mode }: SocialAuthButtonsProps) {
  ...
  const handleSocialAuth = async (provider: OAuthProvider) => {
    ...
    if (authError) {
      setError(authError.message || `Failed to ${mode === 'signin' ? 'sign in' : 'sign up'} with provider`);
      setLoadingProvider(null);
    }
    // Don't reset loading on success - browser is redirecting to the provider
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {providers.map(({ id, name, Icon }) => (
        ...
        >
          {loadingProvider === id ? <LoadingSpinner /> : <Icon />}
          <span>{loadingProvider === id ? 'Redirecting...' : `${actionLabel} with ${name}`}</span>
        </button>
      ))}
    </div>
  );
