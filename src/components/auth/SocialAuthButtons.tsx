...
export default function SocialAuthButtons({ mode }: { mode: 'signin' | 'signup' }) {
  ...
  return (
    <div>
      {error && (
        <div className="mb-3 p-2.5 bg-red-50 border border-red-200 rounded-xl text-red-600 text-xs text-center">
          {error}
        </div>
      )}
      <div className="grid grid-cols-3 gap-2.5">
        {providers.map(({ id, name, Icon }) => (
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-600 hover:bg-white hover:border-slate-300 hover:shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loadingProvider === id ? (
