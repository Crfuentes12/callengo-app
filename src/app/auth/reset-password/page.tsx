...
export default function ResetPasswordPage() {
  ...
  const isValid = password.length >= 6 && confirmPassword.length > 0 && passwordsMatch;
  ...
  const EyeToggle = ({ show, onToggle }: { show: boolean; onToggle: () => void }) => (
    <button type="button" onClick={onToggle} className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 transition-colors" tabIndex={-1}>
      {show ? (
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-slate-400 text-sm mt-1 mb-6">Choose a strong password for your account</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          {password.length > 0 && (
            <div className="flex gap-1 mt-2">
              <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 6 ? 'bg-emerald-400' : 'bg-slate-200'}`} />
              <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 8 ? 'bg-emerald-400' : 'bg-slate-200'}`} />
              <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 10 && /[A-Z]/.test(password) && /[0-9]/.test(password) ? 'bg-emerald-400' : 'bg-slate-200'}`} />
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={`auth-input ${!passwordsMatch ? '!border-red-300 !shadow-[0_0_0_3px_rgba(239,68,68,0.08)]' : ''}`} placeholder="Confirm password" minLength={6} required disabled={loading} autoComplete="new-password" />
          </div>
          {!passwordsMatch && <p className="text-[10px] text-red-500 mt-1">Passwords do not match</p>}
        </div>
