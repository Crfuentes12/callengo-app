...
export default function ForgotPasswordPage() {
  ...
  if (sent) {
    return (
      <div className="animate-fade-in text-center">
        <div className="w-12 h-12 rounded-2xl gradient-bg flex items-center justify-center mx-auto mb-4 shadow-lg shadow-[var(--color-accent)]/20">
        ...
        <h1 className="text-2xl font-bold text-white tracking-tight">{t.auth.forgotPassword.successTitle}</h1>
        <p className="text-white/40 text-sm mt-1">{t.auth.forgotPassword.successSubtitle} <span className="text-white/70 font-medium">{email}</span></p>

        <div className="mt-5 bg-white/[0.05] border border-white/10 rounded-xl p-4 text-left">
          <ol className="space-y-2">
            {[t.auth.forgotPassword.step1, t.auth.forgotPassword.step2, t.auth.forgotPassword.step3].map((step, i) => (
        ...
        <Link href="/auth/login" className="inline-flex items-center gap-1.5 mt-5 text-xs text-white/30 hover:text-white/60 font-medium transition-colors">
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-2xl font-bold text-white tracking-tight">{t.auth.forgotPassword.title}</h1>
      <p className="text-white/40 text-sm mt-1 mb-6">{t.auth.forgotPassword.subtitle}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-white/40 mb-1.5">{t.auth.forgotPassword.emailLabel}</label>
          <div className="relative">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t.auth.forgotPassword.emailPlaceholder} className="auth-input" required disabled={loading} autoComplete="email" />
          </div>
        </div>

        <button type="submit" disabled={loading} className="auth-submit-btn flex items-center justify-center gap-2">
          {loading ? (
            <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{t.auth.forgotPassword.submitting}</>
