...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signInError) {
      setError(signInError.message || t.auth.login.errorInvalidCredentials);
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-white/40 text-sm mt-1 mb-6">{t.auth.login.subtitle}</p>
      ...
      {(error || searchParams.get('error')) && (
        <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-300 text-xs flex items-center gap-2">
          ...
          <span>{error || t.auth.login.errorAuth}</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-white/40 mb-1.5">{t.auth.login.emailLabel}</label>
          <div className="relative">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="auth-input" placeholder={t.auth.login.emailPlaceholder} required disabled={loading} autoComplete="email" />
          </div>
        </div>

        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="block text-xs font-medium text-white/40">{t.auth.login.passwordLabel}</label>
            <Link href="/auth/forgot-password" className="text-xs text-white/50 hover:text-white/70 font-medium transition-colors">{t.auth.login.forgotPassword}</Link>
          </div>
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="auth-input" style={{ paddingRight: '2.5rem' }} placeholder={t.auth.login.passwordPlaceholder} required disabled={loading} autoComplete="current-password" />
        ...
        <button type="submit" disabled={loading} className="auth-submit-btn flex items-center justify-center gap-2">
          {loading ? (
            <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{t.auth.login.submitting}</>
      ...
      <div className="flex items-center my-5">
        ...
        <span className="px-4 text-xs text-white/25">{t.auth.login.or}</span>
        <div className="flex-1 border-t border-white/10" />
      </div>
      ...
      <p className="text-center text-xs text-white/30 mt-5">
        ...
        <Link href="/auth/signup" className="text-white/70 font-semibold hover:text-white transition-colors">{t.auth.login.createAccount}</Link>
      </p>
    </div>
  );
}
