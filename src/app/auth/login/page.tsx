...
function LoginForm() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-slate-400 text-sm mt-1 mb-6">Sign in to your account</p>
      ...
      {searchParams.get('verified') === 'true' && (
        <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-600 text-xs flex items-center gap-2">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="/auth/forgot-password" className="text-xs text-[var(--color-primary)] hover:opacity-80 font-medium">Forgot?</Link>
          </div>
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 transition-colors" tabIndex={-1}>
              {showPassword ? (
      ...
      <p className="text-center text-xs text-slate-400 mt-5">
        ...
        <Link href="/auth/signup" className="text-[var(--color-primary)] font-semibold hover:opacity-80">Create account</Link>
      </p>
    </div>
  );
}
