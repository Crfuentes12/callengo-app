...
// Initiates the Pipedrive OAuth flow

import { NextRequest, NextResponse } from 'next/server';
import { createServerClient } from '@/lib/supabase/server';
import { getPipedriveAuthUrl } from '@/lib/pipedrive';
  ...
  try {
    ...
    if (!['business', 'teams', 'enterprise'].includes(planSlug)) {
      return NextResponse.json(
        { error: 'Pipedrive integration requires Business plan or higher' },
        { status: 403 }
      );
    }
    ...
    console.error('Error initiating Pipedrive connect:', error);
    return NextResponse.json(
