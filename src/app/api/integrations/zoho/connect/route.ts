// app/api/integrations/zoho/connect/route.ts
// Initiates the Zoho CRM OAuth flow
  ...
  try {
    ...
    if (!['business', 'teams', 'enterprise'].includes(planSlug)) {
      return NextResponse.json(
        { error: 'Zoho CRM integration requires Business plan or higher' },
        { status: 403 }
      );
    }
    ...
    console.error('Error initiating Zoho connect:', error);
    return NextResponse.json(
